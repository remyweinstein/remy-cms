...
class Url {
	...
	public static function init() {
		...
		self::$documentRoot = str_replace(DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'classes', '', dirname(__FILE__));
		...
		if($uri!="") {
			...
			switch (count($catalogies)) {
				case 1:
					switch($catalogies[0]) {
						case "admin":
							...
							Engine::$curModule = "dashboard";
							Engine::$curTemplate = "admin";
							break;
						case "login":
							Engine::$curModule = "login";
							break;
						case "registration":
							Engine::$curModule = "registration";
							break;
						default:
				...
				case 2:
					switch($catalogies[0]) {
						case "admin":
							Engine::$curAdmin = true;
							Engine::$curModule = self::clearSection($catalogies[1]);
							Engine::$curTemplate = "admin";
							break;
						case "login":
							Engine::$curModule = "login";
						...
						case "registration":
							...
							break;
						default:
							if(class_exists(self::clearSection($catalogies[0]))) {
								Engine::$curModule = self::clearSection($catalogies[0]);
							}
			...
			if(!class_exists(Engine::$curModule)) {
				Engine::$curModule = 'page';
